...
3. **Brownfield scan.** Dispatch the named [`sdd-explorer`](../../agents/sdd-explorer.md) agent (carries `model: haiku` + `effort: low` — a cheap read-only scan; clean-isolated context per [`../_shared/agent-roster.md`](../_shared/agent-roster.md)): «Map this repo for an architecture document of feature `<slug>`. Report under 400 words: (a) primary language + frameworks + versions, (b) top-level module layout, (c) layering / ports conventions, (d) datastores in use, (e) inter-module communication style (direct call / events / HTTP), (f) anything that constrains `<slug>` (existing tables, endpoints, conventions).» Greenfield (no source) → skip, note `<!-- brownfield: N/A — greenfield repo -->` in §3. (If `sdd-explorer` is unavailable, fall back to a `subagent_type: "Explore"` Agent.)
