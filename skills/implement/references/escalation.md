...
2. **More effort, then more model.** If retries stall, first re-dispatch the GREEN step at `effort: xhigh` on the same model (Sonnet 5 supports the full effort range — raise `effort_implementer` / `CLAUDE_CODE_EFFORT_LEVEL` for this task). If it still stalls, re-dispatch on a stronger model (raise `model_implementer`). A harder task sometimes just needs more reasoning depth, not a bigger model.
