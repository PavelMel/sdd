...
description: >
  ...
  "переглянь зміни {slug}", "код-рев'ю фічі {slug}", "рев'ю diff". Dispatches the sdd-reviewer
...
It reuses the shared clean-context discipline ([`../_shared/critic.md`](../_shared/critic.md)) and the [`sdd-reviewer`](../../agents/sdd-reviewer.md) subagent (read-only). Question phrasing per [`../_shared/ask-style.md`](../_shared/ask-style.md).
...
- Read for the review baseline: `docs/features/<slug>/spec.md` §5 (the AC), `tasks.json` (which AC each task claimed), `data-model.md` / `contracts/openapi.yaml` / Accepted `adr/` (the contracts the code must honour).
...
2. **Dispatch the independent reviewer.** Run [`sdd-reviewer`](../../agents/sdd-reviewer.md) (read-only, **clean context** — it re-reads spec/contracts itself, no paraphrase) over the diff along the dimensions in [`./references/review-dimensions.md`](./references/review-dimensions.md): **stage 1** — every claimed AC genuinely satisfied + no in-scope AC silently uncovered; **stage 2** — conventions, error/edge handling, security, boundary violations, test adequacy. For a large diff, fan out one reviewer per dimension and merge.
...
- Every claimed AC was checked for genuine satisfaction; every in-scope AC is covered or explicitly deferred with owner + due.
...
- [`./references/review-dimensions.md`](./references/review-dimensions.md) — the review dimensions + the sdd-reviewer dispatch shape.
